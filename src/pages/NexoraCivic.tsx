...
const NexoraCivic = () => {
	...
	const ksfTimeline = [
		{
			...
			solution: 'NEXORA.CIVIC automatycznie konfiguruje strukturę uprawnień zgodnie z wymogami KSeF',
		},
		{
	...
	const modules = [
		...
		},
		{
			...
			description: 'Kompleksowe narzędzia finansowe dostosowane do specyfiki sektora publicznej',
			icon: 'pie-chart',
	...
	const securityLayers = [
		{
			...
			description:
				'End-to-end encryption dla wszystkich danych wrażliwych z wykorzystaniem algorytmów AES-256. Wszystkie dane przesyłane między klientem a serwerem są szyfrowane, zapewniając najwyższy poziom bezpieczeństwa informacji obywateli.',
		},
		{
			title: 'Kontrola Dostępu (IAM)',
			description:
				'Wielopoziomowy system zarządzania tożsamością i dostępem z uwierzytelnianiem dwuskładnikowym. Każdy użytkownik ma przypisane konkretne uprawnienia, a dostęp do wrażliwych danych jest ściśle kontrolowany.',
		},
		{
			title: 'System Wykrywania Włamań (IDS/IPS)',
			description:
				'Zaawansowane narzędzia wykrywania i zapobiegania intruzom w czasie rzeczywistym. System monitoruje całą infrastrukturę IT i automatycznie reaguje na potencjalne zagrożenia.',
		},
		{
			title: 'Firewall Aplikacyjny (WAF)',
			description:
				'Zaawansowany firewall aplikacyjny chroniący przed atakami na poziomie aplikacji webowych. Filtruje ruch HTTP/HTTPS i blokuje złośliwe żądania przed dotarciem do serwera.',
		},
		{
			title: 'Kopie Zapasowe i Odzyskiwanie',
			description:
				'Automatyczne, szyfrowane kopie zapasowe z możliwością szybkiego odzyskiwania danych. System zapewnia ciągłość działania nawet w przypadku awarii sprzętowej.',
		},
		{
			title: 'Monitoring i Audyt',
			description:
				'Kompleksowy system monitoringu i audytu wszystkich operacji w systemie. Każda akcja jest logowana i może być przeanalizowana w celach bezpieczeństwa i zgodności.',
		},
		{
			title: 'Plany Ciągłości Działania',
			description:
				'Kompleksowe procedury zapewniające ciągłość świadczenia usług publicznych. Obejmują scenariusze awaryjne, procedury odzyskiwania i plany komunikacji kryzysowej.',
		},
		{
			title: 'Zgodność z RODO i Przepisami',
			description:
				'Pełna zgodność z Rozporządzeniem o Ochronie Danych Osobowych oraz wszystkimi przepisami dotyczącymi bezpieczeństwa informacji w administracji publicznej.',
		},
	]
	...
	const testimonials = [
		{
			quote:
				'NEXORA.CIVIC znacznie uprościł nasze procesy administracyjne. Mieszkańcy doceniają możliwość załatwiania spraw online, a my oszczędzamy czas na rutynowych czynnościach.',
		...
		{
			quote:
				'Dzięki systemowi NEXORA.CIVIC nasza księgowość jest w pełni zgodna z przepisami, a raporty generują się automatycznie. To ogromna oszczędność czasu i pewność zgodności.',
			author: 'Piotr Nowak',
			position: 'Skarbnik',
		...
		{
			quote:
				'Portal mieszkańca to przełom w komunikacji z obywatelami. Znacznie zmniejszyła się liczba wizyt w urzędzie, a satysfakcja mieszkańców wzrosła.',
	...
	const faqData = [
		{
			...
			questions: [
				{
					q: 'Czy NEXORA.CIVIC jest zgodny z dyrektywą NIS2?',
					a: 'Tak, NEXORA.CIVIC w pełni implementuje wszystkie wymagania dyrektywy NIS2, w tym monitoring cyberbezpieczeństwa, raportowanie incydentów i zarządzanie ryzykiem.',
				},
				{
		...
		},
		{
			...
			questions: [
				{
					q: 'Ile czasu zajmuje wdrożenie systemu NEXORA.CIVIC?',
		...
		},
		{
			...
			questions: [
				{
					q: 'Czy NEXORA.CIVIC jest gotowy na KSeF?',
				...
				},
				{
					...
					a: 'NEXORA.CIVIC oferuje narzędzia do migracji danych, automatyczną konfigurację uprawnień, szkolenia dla pracowników oraz dedykowane wsparcie techniczne podczas całego procesu wdrożenia.',
				},
			],
		},
	]
			...
			<section ref={heroRef} className="relative min-h-screen flex items-center bg-[#0D1117] pt-20">
				...
				<div className="container mx-auto px-8 relative z-10 text-center">
					...
					<p className="text-xl text-[#8B949E] max-w-3xl mx-auto leading-relaxed mb-12">
						System NEXORA.CIVIC: Cyfrowy fundament dla nowoczesnego samorządu. Bezpieczne, zgodne z prawem i gotowe na
						przyszłość rozwiązanie dla administracji publicznej.
					</p>
								...
								{ksfTimeline.map((item, idx) => (
									<div key={idx} className="relative flex items-start space-x-8">
										...
										<div className="flex-shrink-0 w-16 h-16 bg-[#3B82F6] border-4 border-[#F0F6FC] rounded-full flex items-center justify-center">
											<LineIcon name="calendar" className="w-6 h-6 text-[#F0F6FC]" />
										</div>
								...
								<div
										...
										<div className="grid grid-cols-2 gap-4">
											{module.features.map((feature, featureIdx) => (
												<div key={featureIdx} className="flex items-center space-x-2">
													<LineIcon name="checkmark-circle" className="w-5 h-5 text-[#3B82F6] flex-shrink-0" />
									...
									<div className={`${idx % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''} flex justify-center`}>
										<div className="w-64 h-64 bg-[#3B82F6]/10 border-4 border-[#3B82F6] flex items-center justify-center">
											<LineIcon name={module.icon} className="w-32 h-32 text-[#3B82F6]" />
										</div>
									</div>
								</div>
							</div>
						...
						{modules.map((module, idx) => (
							<div
								...
								<div className="flex flex-col h-full justify-between">
									...
									<LineIcon
										name="arrow-right"
										className="w-6 h-6 text-[#3B82F6] mt-4 group-hover:translate-x-2 transition-transform duration-300"
									/>
								</div>
							</div>
						))}
						...
						<div ref={securityTextRef} className="lg:h-screen flex flex-col justify-center">
							...
							<div className="relative w-full h-[400px] flex items-center justify-center pb-28">
								...
								<div className="relative w-80 h-80">
									...
									{/* Middle ring */}
									<div
										className="absolute inset-8 border-4 border-[#3B82F6]/70 rounded-full animate-pulse"
										style={{ animationDelay: '0.5s' }}></div>
									{/* Inner ring */}
									<div
										className="absolute inset-16 border-4 border-[#3B82F6]/40 rounded-full animate-pulse"
										style={{ animationDelay: '1s' }}></div>
									{/* Center */}
									<div className="absolute inset-0 flex items-center justify-center">
										<LineIcon name="shield" className="w-24 h-24 text-[#3B82F6]" />
									</div>
						...
						{testimonials.map((_, idx) => (
							<button
								...
								className={`w-4 h-4 rounded-full transition-all duration-300 hover:scale-125 ${
									currentTestimonial === idx ? 'bg-[#3B82F6]' : 'bg-[#21262D] hover:bg-[#3B82F6]/50'
								}`}
							/>
						))}
					</div>
				...
				<div className="container mx-auto px-8">
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
						<div className="text-center">
							<div className="text-6xl font-bold text-[#F0F6FC] mb-4 counter" data-target="300">
								300
							</div>
							<div className="text-xl text-[#F0F6FC]/90">Jednostek JST</div>
						</div>
						<div className="text-center">
							<div className="text-6xl font-bold text-[#F0F6FC] mb-4 counter" data-target="60">
								60
							</div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
						...
						<div className="space-y-8">
							...
							<p className="text-xl text-[#8B949E] leading-relaxed">
								Dołącz do grona nowoczesnych samorządów, które już korzystają z systemu NEXORA.CIVIC. Umów się na
								bezpłatną prezentację i zobacz jak możemy pomóc Twojemu urzędowi.
							</p>
						...
						<div className="bg-[#F0F6FC] border-4 border-[#3B82F6] p-8">
							...
							<div className="space-y-4">
								<div className="flex items-start space-x-4">
									<LineIcon name="apartment" className="w-6 h-6 text-[#3B82F6] flex-shrink-0 mt-1" />
									<div>
										<p className="font-bold text-[#0D1117]">Dział Administracji Publicznej</p>
										<p className="text-[#21262D]">
											ul. Samorządowa 35
											<br />
											40-000 Katowice
										</p>
								...
								<div className="flex items-start space-x-4">
									<LineIcon name="users" className="w-6 h-6 text-[#3B82F6] flex-shrink-0 mt-1" />
									<div>
								...
								<div className="flex items-start space-x-4">
									<LineIcon name="world" className="w-6 h-6 text-[#3B82F6] flex-shrink-0 mt-1" />
									<div>
										<p className="font-bold text-[#0D1117]">Email</p>
										<p className="text-[#21262D]">civic@nexora.pl</p>
									</div>
								</div>
							</div>
						</div>
