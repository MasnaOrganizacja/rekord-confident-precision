...
export const FAQ = () => {
	const [category, setCategory] = useState<'general' | 'flow' | 'civic'>('general')

	const faqs = {
		general: [
			{
				q: 'Jak długo trwa typowe wdrożenie systemu?',
				a: 'Czas wdrożenia zależy od rozmiaru organizacji i zakresu funkcjonalności. Dla średniej firmy typowe wdrożenie NEXORA.FLOW trwa 3-6 miesięcy, natomiast NEXORA.CIVIC dla JST to 4-8 miesięcy. Każde wdrożenie poprzedzamy szczegółową analizą i dostarczamy harmonogram dostosowany do specyfiki klienta.',
			},
			{
		...
		flow: [
			{
				q: 'Czy NEXORA.FLOW obsługuje produkcję wieloseryjną?',
			...
			},
			{
				...
				a: 'Tak, NEXORA.FLOW pozwala zarządzać nieograniczoną liczbą magazynów w różnych lokalizacjach. Transfer między magazynami, inwentaryzacje, śledzenie partii i numerów seryjnych, zarządzanie stanami minimalnymi i maksymalnymi – wszystko w jednym systemie.',
			},
			{
			...
			{
				q: "Czy NEXORA.FLOW wspiera CRM i zarządzanie lead'ami?",
		...
		civic: [
			{
				q: 'Czy NEXORA.CIVIC jest zgodny z wymogami KSeF?',
				a: 'Tak, NEXORA.CIVIC jest w pełni gotowy do obsługi Krajowego Systemu e-Faktur. Automatyczna walidacja, wysyłka i odbiór faktur ustrukturyzowanych, integracja z systemem finansowym, archiwizacja zgodna z przepisami. Zapewniamy pełne wsparcie w procesie wdrożenia KSeF.',
			},
			{
				q: 'Czy system obsługuje e-PUAP i portal ePUAP?',
				a: 'Tak, NEXORA.CIVIC jest zintegrowany z platformą ePUAP. Automatyczny odbiór pism z ePUAP, elektroniczny obieg dokumentów, podpis elektroniczny i wysyłka odpowiedzi. Pełna zgodność z wymogami Krajowych Ram Interoperacyjności.',
			},
			{
			...
			{
				q: 'Czy NEXORA.CIVIC spełnia wymogi RODO dla jednostek publicznych?',
